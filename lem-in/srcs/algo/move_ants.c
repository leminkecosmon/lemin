...
/*   Updated: 2019/03/20 14:02:50 by agesp            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_ratio_to_paths(t_path *p)
{
	int		min;
	float	ratio;
	...
	while (p)
	{
		ratio = (float)p->size_path / (float)min;
		p->ratio = (int)ratio;
		if (!p->next)
			break ;
		p = p->next;
...
void	set_first_ants(t_lemin *e, t_path *p)
{
	t_ants *a;
...
void	move_ants_forward(t_lemin *e)
{
	...
	free_path(&p);
	p = e->p;
	// GENRE A CETTE ENDROIT LA, T'AS QUOI ??
